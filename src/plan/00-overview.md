...
- 현재 구현에서는 팀장/팀원 분리 없이 단일 에이전트가 계획과 실행을 모두 담당
...
- MCP/FastMCP 기반 스킬 시스템은 문서상 미래 확장 방향이며, 현재 구현의 도구 호출은 `app/services/agent.py` 내부 함수로 직접 처리한다.
