...
def build_stable_tool_contract() -> str:
    lines = [
        "도구 사용 공통 계약:",
        "- 현재 turn에서 실제로 사용할 수 있는 도구 목록과 도구별 세부 규칙은 runtime context에 제공됩니다.",
        "- runtime context의 사용 가능한 도구 목록에 없는 도구 이름은 절대 호출하지 마세요.",
        "- 필요한 도구가 runtime context에 없으면 현재 가능한 범위만 텍스트로 설명하고 멈추세요.",
        "",
        "채팅 작업공간 공통 규칙:",
        ...
        "",
        "파일 DB 조회 공통 규칙:",
        "- 파일/폴더 검색, 개수, 요약 기반 탐색은 runtime context에 제공된 SQLite 파일 DB 도구만 사용하세요.",
        ...
        "",
        "도구 호출 출력 계약:",
        "- 도구를 사용할 때는 반드시 fenced block 형식만 사용하세요.",
        ...
        "작업 계획을 먼저 설명한 뒤 도구를 호출해야 한다면, 마지막은 반드시 runtime context의 도구 호출 형식으로 끝내세요.",
        "한 번에 하나의 도구만 호출하세요.",
...
def build_dynamic_tool_context(selected_tools: list[str] | None) -> str:
    tools = normalize_tool_names(selected_tools)
    if not tools:
        tools = []
    example_tool = tools[0] if tools else "file_search"

    lines = ["이번 turn에서 사용 가능한 도구 목록:"]
    if tools:
