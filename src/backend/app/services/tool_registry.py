...
TOOL_CATALOG: dict[str, dict[str, object]] = {
    "file_create": {
        ...
        "description": "file_create(path, content) - 새 파일 생성. 저장 위치가 명확하지 않으면 현재 채팅 outputs/ 아래에 배치됩니다.",
    },
    "file_read": {
        "purpose": "파일 내용 읽기",
    ...
    "file_write": {
        ...
        "description": "file_write(path, content) - 기존 파일 덮어쓰기.",
    },
    "file_delete": {
        "purpose": "파일 삭제",
...
def _tool_call_example(tool_name: str) -> str:
    examples = {
        ...
        "file_create": '{"tool": "file_create", "args": {"path": "dashboard.md", "content": "# 제목"}}',
        "file_read": '{"tool": "file_read", "args": {"path": "outputs/report.md"}}',
        "file_write": '{"tool": "file_write", "args": {"path": "outputs/report.md", "content": "# 수정된 내용"}}',
...
def build_tool_descriptions(selected_tools: list[str] | None) -> str:
    ...
    lines.extend([
        "",
        "채팅 작업공간 규칙:",
        "- 사용자가 저장 위치를 명확히 말하지 않았다면 파일 생성/수정/폴더 생성은 현재 채팅 작업공간에 저장하세요.",
        "- 사용자가 파일 생성/저장/수정/삭제를 명시적으로 요청하지 않은 질문에는 파일 변경 도구를 호출하지 말고 텍스트로만 답하세요.",
        "- 명시 경로 없이 파일명만 필요하면 상대 경로 또는 파일명만 path에 넣으세요. 백엔드가 현재 채팅 outputs/working으로 배치합니다.",
