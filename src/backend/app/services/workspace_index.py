...
async def update_file_summary(workspace: str, file_path: str, action: str) -> None:
    ...
    openclaw = os.path.join(workspace, ".openclaw")
    summaries_dir = os.path.join(openclaw, "file_summaries")
...
async def rebuild_workspace_index(workspace: str) -> None:
    ...
    with open(os.path.join(openclaw, "workspace.md"), "w", encoding="utf-8") as f:
        f.write(content)


def load_workspace_context(workspace: str) -> str | None:
    """workspace.md 로드"""
    path = os.path.join(workspace, ".openclaw", "workspace.md")
