...
haro는 프로젝트별 워크스페이스를 만들고, AI 에이전트가 파일을 읽고 수정하며, 사용자가 브라우저 UI에서 파일을 탐색, 미리보기, 편집, 업로드할 수 있는 POC 개발 환경이다.
...
기존 POC에서 생성된 `.openclaw` 디렉토리는 읽기 fallback으로 지원하지만, 신규 인덱스는 `.haro`에 기록한다.
